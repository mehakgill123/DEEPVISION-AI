...
from .models import CustomUser, ContactUs, Feedback
...
def create_contactus(request):
    try:
        ...
        ContactUs.objects.create(name=name, email=email, message=message, subject=subject)

        return JsonResponse(
            {'status': 'success', 'message': 'Contact us message created successfully.'},
            status=201
        )
    except json.JSONDecodeError:
        return JsonResponse(
...
def add_feedback(request):
    try:
        ...
        Feedback.objects.create(comment=comment, rating=rating, user=user)

        return JsonResponse(
            {'status': 'success', 'message': 'Feedback created successfully.'},
            status=201
        )
    except json.JSONDecodeError:
        return JsonResponse(
